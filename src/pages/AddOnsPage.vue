<!--
  AddOnsPage.vue
  ─────────────────────────────────────────────────────────────────────────
  Replaces MarketplaceFrame.vue's iframe-of-mp.gohighlevel.com with a
  native Vue surface so we own the design and the data.

  Shell mirrors MarketplaceFrame.vue exactly:
    <div>
      <SideBarV2 />
      <TopBar />
      <section class="hl_wrapper">…page body…</section>
      <KickoffWidget />
    </div>

  HighRise usage is intentionally narrow — only HLTabs / HLTabPane.
  Card surface, tag, and CTA are bespoke markup so we get pixel-level
  control matching the design mock. HighRise v3.4.1's HLCard / HLButton
  / HLTag don't render the visual treatment the design needs, and
  fighting their internals costs more than styling 60 lines of CSS.
  Tokens (colors, radii, type scale) follow HighRise gray/primary/
  success conventions documented in the design system.

  Copy is hardcoded for now. Move to vue-i18n keys (addons.* namespace)
...
<script lang="ts" setup>
  ...
  // Preview-repo shell + HighRise stubs.
  // Source-of-truth for this page = spm-ts/src/pmd/pages/agency/AddOnsPage.vue.
  // The ONLY differences from the prod file are this import block and the
  // template wrapper (<ShellV1> instead of <SideBarV2>/<TopBar>/<KickoffWidget>).
  // Everything below — copy, styles, structure — is byte-identical to prod.
...
<template>
  <ShellV1 active="addons">
    <!--
      ...
      omitted here because it depends on Vuex + onboarding state
      that doesn't exist in the preview repo. Visually it's a small
      bottom-right widget; not worth stubbing for a static preview.
    -->
...
<style scoped>
   ...
   *
   * NOTE: a few hex values I used are visual neighbors of HR tokens
   * (e.g. mine var(--success-200) → HR --success-200 var(--success-200)). Diff is
   * imperceptible and within DS tolerance.
   */
  ...
  /*
   * Primary CTA uses HighRise's --primary-600 (var(--primary-600)) — the brand
   * blue. Black/navy isn't a brand color in spm-ts; the production
